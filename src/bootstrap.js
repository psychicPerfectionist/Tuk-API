// bootstrap.js - Auto-seeds the database on first startup (empty DB)
// On Railway (ephemeral filesystem) this runs on every cold deploy.
// On a persistent volume it only runs once.
...
const padL   = (n, w) => String(n).padStart(w, '0');

function generateNIC() {
  return `${rndInt(1970,1999)}${padL(rndInt(1,366),3)}${padL(rndInt(1000,9999),4)}V`;
}

function generateRegPlate(provCode, seq) {
  const L = 'ABCDEFGHJKLMNPQRSTUVWXYZ';
  return `${provCode} ${pick(L)}${pick(L)}${pick(L)}-${padL(seq,4)}`;
}
...
function generateMovementLog(vehicleId, centreLat, centreLng, weekAgo) {
  ...
  for (let day = 7; day >= 0; day--) {
    const dayStart = new Date(weekAgo + day * 86400000);
    if (dayStart.getDay() === 0 && Math.random() < 0.3) continue;

    const startH = rndInt(5, 8);
    const endH   = rndInt(19, 22);
    const workStart = new Date(dayStart); workStart.setHours(startH, rndInt(0,59), 0, 0);
    const workEnd   = new Date(dayStart); workEnd.setHours(endH,   rndInt(0,59), 0, 0);

    let lat = centreLat + rnd(-0.03, 0.03);
    let lng = centreLng + rnd(-0.03, 0.03);
    let t   = workStart.getTime();

    while (t <= workEnd.getTime() && t <= now) {
      lat = Math.max(5.9, Math.min(9.9, lat + rnd(-0.002, 0.002)));
      lng = Math.max(79.5, Math.min(81.9, lng + rnd(-0.002, 0.002)));
      pings.push({
        vehicleId,
      ...
      t += pingInterval + rndInt(-60000, 60000);
    }
  }
  return pings;
}
...
async function bootstrap() {
  const userCount = await dbAsync.users.count({});
  if (userCount > 0) return; // already seeded

  console.log('\n🌱 Empty database detected — running first-time seed...\n');
  ...
  for (const s of STATIONS_DATA) {
    const provCode = districtProvMap[s.district];
    const doc = await dbAsync.stations.insert({
      name: s.name, code: s.code,
      districtId: districtMap[s.district]._id,
      provinceId: provinceMap[provCode]._id,
      stationType: s.type,
      address: `${s.name}, ${districtMap[s.district].name}, Sri Lanka`,
      contactNumber: s.phone,
      coordinates: { latitude: s.lat, longitude: s.lng },
      isActive: true,
      createdAt: new Date().toISOString(),
    });
    stationMap[s.code] = { ...doc, districtCode: s.district, provCode };
  }
  ...
  for (let i = 0; i < 220; i++) {
    ...
    const doc = await dbAsync.drivers.insert({
      fullName:      `${pick(DRIVER_FIRST)} ${pick(DRIVER_LAST)}`,
      licenseNumber: `B${padL(rndInt(1000000, 9999999), 7)}`,
      licenseExpiry: `${expYear}-${padL(rndInt(1,12),2)}-${padL(rndInt(1,28),2)}`,
      nicNumber:     generateNIC(),
      dateOfBirth:   `${year}-${padL(rndInt(1,12),2)}-${padL(rndInt(1,28),2)}`,
  ...
  for (let i = 1; i <= 210; i++) {
    ...
    const doc = await dbAsync.vehicles.insert({
      ...
      deviceId:    `DEV-${padL(i, 4)}`,
      deviceImei:  `${rndInt(100000000000000, 999999999999999)}`,
      make,
      model:       pick(VEHICLE_MODELS[make]),
      colour:      pick(COLOURS),
      yearOfRegistration: rndInt(2012, 2023),
      engineNumber:  `ENG${padL(rndInt(100000,999999),6)}`,
      chassisNumber: `CHS${padL(rndInt(100000000,999999999),9)}`,
      driverId:    driver._id,
  ...
  console.log('   Generating 1-week GPS history (this may take ~30s)...');
  ...
  for (const vehicle of vehicleDocs.filter(v => v.status === 'ACTIVE')) {
    ...
    for (const ping of pings) await dbAsync.locations.insert(ping);
    totalPings += pings.length;
    if (pings.length > 0) {
      const last = pings[pings.length - 1];
      await dbAsync.vehicles.update({ _id: vehicle._id }, {
        $set: { lastLocation: { latitude: last.latitude, longitude: last.longitude, timestamp: last.timestamp } },
      });
    }
  }
  ...
  console.log(`\n✅ Bootstrap complete — ${totalPings.toLocaleString()} location pings, 210 vehicles, 9 users seeded.`);
  console.log('   Credentials: admin / Admin@1234\n');
}
